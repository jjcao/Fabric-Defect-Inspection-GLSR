...
%       'u2'   for uniform LBP
%       'ri'   for rotation-invariant LBP  最值36
%       'riu2' for uniform rotation-invariant LBP.  10种类型
%
%  Example:
%       I=imread('rice.tif');
...
if strcmp(mappingtype,'u2') %Uniform 2
  newMax = samples*(samples-1) + 3; %59种
  for i = 0:2^samples-1
     
    j = bitset(bitshift(i,1,key),1,bitget(i,samples)); %rotate left  将i左移一位 然后将i与j异或 得到0-1 1-0的个数
...
if strcmp(mappingtype,'ri') %Rotation invariant
  ...
  for i = 0:2^samples-1
    ...
    r  = i;
    for j = 1:samples-1
      r = bitset(bitshift(r,1,key),1,bitget(r,samples)); %rotate
                                                        %left
      if r < rm
        rm = r;
      end
...
if strcmp(mappingtype,'riu2') %Uniform & Rotation invariant
  newMax = samples + 2;
  for i = 0:2^samples - 1
    j = bitset(bitshift(i,1,key),1,bitget(i,samples)); %rotate left
